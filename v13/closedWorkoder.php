...
INNER JOIN sv_ticket_details ON tbcustomer.sv_number = sv_ticket_details.sv_number WHERE sv_ticket_header.sv_number = ".$id." AND sv_ticket_header.log_ID=".$log."";
...
if(isset($_POST['postComment'])){
    ...
    $MessageReplay = "[ SYSTEM : WORKORDER CLOSED ]: ".htmlspecialchars( $_POST['commentbox'],  ENT_QUOTES );
    ...
    if(isset($MessageReplay)){
		...
		$UpdateWOClose = DB::getInstance()->prepare("UPDATE ticket_status SET isWOClosed = 1,isWOOpen = 0,dateWOClosed = now() 
		WHERE ticketNo = ? AND log_ID = ?");
	    $UpdateWOClose->execute(array($ticketNumber,$log));
        ...
        if(isset($status)){
            ...
            switch($status){
                case 1:
                    Redirect::to( 5, 'closedWorkoder?id='.$ticketNumber.'&&log='.$log.'' );
                    break;
                case 0:
...
<head>
    ...
    <meta charset="utf-8">
	<title>Workorder - Service Ticket</title>
           ...
           <!-- START OF NEW TICKET AND USER INFORMATION!! -->
            <div id="tickets">
            	<table style="margin-left:auto; margin-right:auto;">
                	<br />
            		<tr>
                    	<th id="opentickets" style="text-align:center">Close Work Order</th>
                    </tr>
                    <tr>
...
<div id="ticketsScroll">
          ...
          <form action="closedWorkoder.php" method="post">
