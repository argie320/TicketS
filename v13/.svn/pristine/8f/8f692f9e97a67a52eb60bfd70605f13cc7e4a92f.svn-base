...
INNER JOIN tb_return ON tbcustomer.sv_number = tb_return.sv_number
INNER JOIN gagamit ON wo_ticket_details.assignToEngineer = gagamit.email
WHERE tbcustomer.sv_number = ? AND sv_ticket_item_details.sv_number = ? AND tb_return.log=?");
  ...
  //	$result = mysqli_query($dbc, $sql);
	...
	if($result->rowCount()){
         ...
         while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
				...
				$returnRemarks = $row['r_remarks'];
				$r_prepared=$row['r_preparedBy'];
				$r_return=$row['returnBy'];
				$retunDate=$row['returnDate'];
				$r_resolution = $row['problem_res'];
...
if($returnNo==0){
	//include"includes/alert.php";
		 ...
		 echo "alert('Ticket number $id has no returnslip issued');"; 
...
$reportNameYPos = 160;
$logoFile = "http://aldona.sportjunk.net/uploads/wolf_1281110423.jpg";
$logoXPos = 50;
$logoYPos = 108;
$logoWidth = 110;
...
$pdf->Cell( 20, 5, "Return No. :", 1, 0, 'L');
$pdf->Cell( 20, 5, $returnCHK, 1, 0, 'C');
$pdf->Ln( 5 );
...
$pdf->SetFont('Arial','',9);
$pdf->SetTextColor(0,0,0);
  $pdf->CellFitScale( 30, 5, ($itemBrand), 1,  'C');
...
$pdf->Ln( 8 );
$pdf->SetTextColor(0,0,0);
  $pdf->Cell( 190, 5,"Problem Resolution", 1, 0, 'C');
  $pdf->Ln( 8 );
$pdf->SetDrawColor(255,255,255);
$pdf->SetFont( 'Arial', '', 10 );
 $pdf->MultiCell( 190, 5,$r_resolution, 1,'C');            
...
$pdf->SetDrawColor(255,255,255);
$pdf->SetFont( 'Arial', '', 10 );
 $pdf->MultiCell( 190, 5,$returnRemarks, 1,'C');            
  $pdf->Ln(30);
//////////////// acknowlegement /////////////////

$pdf->SetFont( 'Arial', '', 10 );
$pdf->Write( 6, "Prepared By : ".$r_prepared."");
    $pdf->Ln(10);
$pdf->Write( 6,"Date : ".$retunDate."");
    $pdf->Ln(10);
$pdf->Write( 6, "Pullout By : ".$r_return."");
 $pdf->Ln(30);
